...
export function MenuPublico() {
    ...
    useEffect(() => {
        fetch("ec2-3-145-48-161.us-east-2.compute.amazonaws.com:8080/api/platos")
            .then(res => res.json())
                ...
                <div className="menu">
                    {platosNormales.map(plato => (
                        <div key={plato.id} className="menu-card">
                            <img src={`ec2-3-145-48-161.us-east-2.compute.amazonaws.com:8080/api/platos/${plato.id}/imagen`} alt={plato.name} />
                            <h3>{plato.name}</h3>
